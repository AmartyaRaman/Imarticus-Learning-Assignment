...
export default function CourseCard({ onOpenPDFPopup, currentDocument }) {
  ...
  const handleViewDocument = () => {
    const documentUrl = courseDetails[0]?.document || currentDocument;
    if (documentUrl && onOpenPDFPopup) {
      onOpenPDFPopup(documentUrl);
    }
  };
          ...
          <div className="d-flex mb-4">
            ...
            <div style={{ flex: '3' }} className="d-flex flex-column justify-content-between">
              ...
              <div className="align-self-end d-flex gap-2">
                {/* Document Button - Show if document exists */}
                {(courseDetails[0]?.document || currentDocument) && (
                  <button 
                    className="btn btn-outline-primary px-3 py-2"
                    onClick={handleViewDocument}
                    title="View Course Document"
                  >
